...
def calculate_deadline_rigorous(start_date, target_business_days, suspensions, adjust_weekend=True):
    ...
    # Avançamos no tempo até completarmos os dias úteis necessários
    while days_counted < target_business_days:
        current_date += timedelta(days=1)
        
        # Só incrementamos o contador se for um dia válido para o processo correr
...
def calculate_all_milestones(start_date, suspensions, manual_meeting_date=None, adjust_weekend=True):
    # Definição dos marcos em DIAS ÚTEIS
    milestones_def = [
    ...
    # Calcular dias totais de suspensão para info
    total_susp_days = 0
    for s in suspensions:
